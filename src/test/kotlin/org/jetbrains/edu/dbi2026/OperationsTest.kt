...
class OperationsTest  {
    ...
    fun doTestSorting(sorter: MultiwayMergeSort, accessMethodManager: SimpleStorageAccessManager, cache: PageCache) {
        val fooOid = accessMethodManager.createTable("foo")
        TableBuilder(accessMethodManager, cache, fooOid).use {builder ->
            (1..10000).shuffled().forEach {
                builder.insert(intField().first.asBytes(it))
            }
        }
        var sortOutputTableName = ""
        sorter.use { sorter ->
            sortOutputTableName = sorter.sort("foo") {
                intField().first.fromBytes(it).first
            }
            assertEquals((1..10000).toList(), accessMethodManager.createFullScan(sortOutputTableName).records {
                intField().first.fromBytes(it).first
            }.toList())
        }
    ...
    fun `merge sort smoke test`() {
        ...
        doTestSorting(Operations.sortFactory(accessMethodManager, cache), accessMethodManager, cache)
    }
    ...
    fun `join smoke test`() {
        ...
        JoinAlgorithm.values().forEach {joinAlgorithm ->
            Operations.innerJoinFactory(accessMethodManager, cache, joinAlgorithm).map { it ->
                ...
                if (it is NotImplementedError) return@onFailure
                fail(it.message)
            }
        }
    }
