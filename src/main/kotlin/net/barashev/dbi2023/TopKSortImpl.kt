...
class TopKSortImpl(
     ...
     * very first one, could use bulk reads. StorageAccessManager exposes neither the table oid nor the page ids,
     * so we read the catalog the same way SimpleStorageAccessManager does.
     *
     * @return the input page ids or null if the table oid cannot be found, in which case the caller falls back
     *         to the regular full scan
     */
    private fun inputPageIds(tableName: String): List<PageId>? =
        FullScanImpl(cache, NAME_SYSTABLE_OID) { RootRecordIteratorImpl(cache, NAME_SYSTABLE_OID, 1) }
            .records { OidNameRecord(intField(), stringField(), booleanField()).fromBytes(it) }
            .firstOrNull { it.value2 == tableName && !it.value3 }
            ?.value1
            ?.let { oid -> RootRecords(cache, oid, 1).filter { it.value1 == oid }.map { it.value2 } }
