...
enum class CacheAlgorithm {
    NONE, RANDOM, LRU, FIFO, CLOCK, AGING;

    companion object {
        fun fromString(name: String): CacheAlgorithm = when (name.uppercase()) {
            "FIFO" -> FIFO
            "LRU" -> LRU
            "CLOCK" -> CLOCK
            "AGING" -> AGING
...
interface PageCache {
    ...
    var monitoring: CacheMonitoring
    /**
...
interface CacheMonitoring {
    fun recordCacheResult(pageId: PageId, isCacheHit: Boolean)
}
