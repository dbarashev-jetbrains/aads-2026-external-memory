...
package org.jetbrains.edu.dbi2026.fake
 ...
 */
class InMemorySort(private val storageAccessManager: StorageAccessManager, private val cache: PageCache):
    MultiwayMergeSort {
    private val outputTableName = "output${Random.nextLong()}"
    override fun <T : Comparable<T>> sort(tableName: String, comparableValue: Function<ByteArray, T>): String {
        ...
        val outOid = storageAccessManager.createTable(outputTableName)
    ...
    override fun close() {
        super.close()
        storageAccessManager.deleteTable(outputTableName)
    }
}
...
class FakeHashTableBuilder(private val storageAccessManager: StorageAccessManager, private val cache: PageCache): HashtableBuilder {
    ...
    override fun <T> hash(tableName: String, bucketCount: Int, hashKey: Function<ByteArray, T>): Hashtable<T> {
        ...
        repeat(bucketCount) {
            bucketTable.add(
                Bucket(it, "${tableName}_hash_${Random.nextInt()}_${it}", 1)
                    to mutableListOf()
            )
        }
