 ...
 */
abstract class FullScanIteratorBase<T>(
...
/**
 * Iterates over the records of pages in the specified chunk.
 * Returns pairs of (parsed record, raw record bytes). Prevents the order of records on every page and the order of pages.
 */
class ChunkedPageRecords<T>(chunk: List<CachedPage>,
                            recordParser: Function<ByteArray, T>)
    : FullScanIteratorBase<Pair<T, ByteArray>>({ recordParser.apply(it) to it }) {
    private val chunkIterator = chunk.iterator()
    init {
        advance()
    }
    override fun advancePage(): CachedPage? {
