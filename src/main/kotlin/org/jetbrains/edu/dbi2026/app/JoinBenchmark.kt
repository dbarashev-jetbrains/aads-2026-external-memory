...
class JoinBenchmark: CliktCommand() {
    ...
    val realHash by option(help="Use the real hash implementation").flag(default = false)
    val realSort by option(help="Use the real merge-sort implementation").flag(default = false)
    val joinAlgorithm by option(help="Join algorithm to use").enum<JoinAlgorithm>().default(JoinAlgorithm.NESTED_LOOPS)
    override fun run() = try {
        ...
        Operations.innerJoinFactory(accessManager, cache, joinAlgorithm).fold(
            ...
            onSuccess = {
                ...
                println("RANDOM READS:${storage.stats.randomReadCount}")
                println("RANDOM WRITES:${storage.stats.randomWriteCount}")
                println("ACCESS COST:${storage.totalAccessCost - cost0}")
